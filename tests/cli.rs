...
#[test]
fn hash_one_file() -> Result<(), Box<dyn std::error::Error>> {
    let tmp_dir = set_up()?;

    let mut cmd = Command::cargo_bin("dgstore")?;
    cmd.current_dir(&tmp_dir).arg("over-the-rainbow.txt");

    cmd.assert()
        .success()
        .stdout(predicate::str::contains("over-the-rainbow.txt"));
    ...
    expected_state.extend(hashmap! {
        String::from("over-the-rainbow.txt.sha512") => String::from("49735c48046038e54ff7fca9afea03aae18eac08b2e395289a867b221e094dfdca0ad6af3be2b2cc2f950a48a1b4fe61d48ed5d612c797116c39d9e82b5a9829")
    });
...
fn hash_multiple_files_matching_pattern() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cmd = Command::cargo_bin("dgstore")?;
    cmd.current_dir(&tmp_dir).arg("**/*.txt");

    cmd.assert().success().stdout(
        predicate::str::contains("hello/world.txt")
            .and(predicate::str::contains("over-the-rainbow.txt")),
    );
    ...
    let mut expected_state = test_data();
    expected_state.extend(hashmap! {
        String::from("hello/world.txt.sha512") => String::from("921618bc6d9f8059437c5e0397b13f973ab7c7a7b81f0ca31b70bf448fd800a460b67efda0020088bc97bf7d9da97a9e2ce7b20d46e066462ec44cf60284f9a7"),
        String::from("over-the-rainbow.txt.sha512") => String::from("49735c48046038e54ff7fca9afea03aae18eac08b2e395289a867b221e094dfdca0ad6af3be2b2cc2f950a48a1b4fe61d48ed5d612c797116c39d9e82b5a9829")
    });
...
fn recursively_hash_all_files_in_directory() -> Result<(), Box<dyn std::error::Error>> {
    ...
    expected_state.extend(hashmap! {
        String::from("hello/world.txt.sha512") => String::from("921618bc6d9f8059437c5e0397b13f973ab7c7a7b81f0ca31b70bf448fd800a460b67efda0020088bc97bf7d9da97a9e2ce7b20d46e066462ec44cf60284f9a7"),
        String::from("empty.sha512") => String::from("cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce47d0d13c5d85f2b0ff8318d2877eec2f63b931bd47417a81a538327af927da3e"),
        String::from("over-the-rainbow.txt.sha512") => String::from("49735c48046038e54ff7fca9afea03aae18eac08b2e395289a867b221e094dfdca0ad6af3be2b2cc2f950a48a1b4fe61d48ed5d612c797116c39d9e82b5a9829"),
        String::from("random.sha512") => String::from("6f47499789223ed305a19ca1e5050e2c778a0dde0498268b1053ed82334ba44ba79d56d16c9e4c744bc2237798baca2fea39e74c941970b9344d9e64f209f99d")
    });
...
fn test_data() -> HashMap<String, String> {
    ...
    data.insert(String::from("random"), String::from("83c2NQPHOOWUh0JEFWRjdeiZVVRwbqRWDcOe6O7zUM1dJNJBAUReMrRUem2oQhhI9F5uIuOpb30m6rITCO9x5Gb5fnwGKYXOtWNE"));
